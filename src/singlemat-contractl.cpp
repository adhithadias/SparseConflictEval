...
int computeexpr(taco_tensor_t *y, taco_tensor_t *A) {
    ...
    // unordered_map<int, double> y_map;
    // y_map.reserve(y1_dimension);
    double* y_map = (double*)malloc(y1_dimension * sizeof(double));
    bool* y_map_mask = (bool*)calloc(y1_dimension, sizeof(bool));
    int32_t* indices = (int32_t*)malloc(y1_dimension * sizeof(int32_t));
    int count = 0;
    ...
    qsort(indices, count, sizeof(int32_t), compare);
    int32_t jy = 0;
    for (auto it = 0; it < count; it++) {
        int32_t j = indices[it];
        y_vals[it] = y_map[j];
        jy++;
        y_map_mask[j] = false;
    }
    ...
    // unordered_map<int, double> y_map;
    // vector<bool> y_map_mask(y1_dimension);
    ...
    // int count = 0;
    // for (auto it = 0; it < y1_dimension; it++) {
    //     if (y_map_mask[it]) {
    //         y_vals[count] = y_map[it];
    //         count++;
    //     }
    // }
