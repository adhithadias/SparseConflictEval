...
float test_csr_csc(char* file_name) {
  ...
  // Tensor<double> A_({B->dimensions[1], B->dimensions[0]}, Format({Dense, Sparse}));
  // taco_tensor_t* A = A_.getStorage();
  float acc_time = 0;
  for (int i = 0; i < 1; ++i) {
    clear_cache();
    auto tic = TI;
    if (!evaluate_csr_csc(A, B)) {
      ...
      acc_time += toc-tic;
      deinit_taco_tensor_t(A);
      // A_ = Tensor<double>({B->dimensions[1], B->dimensions[0]}, Format({Dense, Sparse}));
      // A = A_.getStorage();
...
float test_coo_csr(char* file_name) {
  ...
  std::vector<taco_mode_t> bufferModeType = {taco_mode_dense, taco_mode_sparse};
  // taco_tensor_t* A = init_taco_tensor_t(2, 8, bufferDim.data(), bufferModeOrdering.data(), bufferModeType.data(), nullptr);
  Tensor<double> A_({B->dimensions[0], B->dimensions[1]}, Format({Dense, Sparse}));
  taco_tensor_t* A = A_.getStorage();
  float acc_time = 0;
  for (int i = 0; i < 1; ++i) {
    clear_cache();
    ...
    if (!evaluate_coo_csr(A, B)) {
      ...
      acc_time += toc-tic;
      deinit_taco_tensor_t(A);
      A_ = Tensor<double>({B->dimensions[0], B->dimensions[1]}, Format({Dense, Sparse}));
      A = A_.getStorage();
      // A = init_taco_tensor_t(2, 8, bufferDim.data(), bufferModeOrdering.data(), bufferModeType.data(), nullptr);
    }
  }
